...
from persistence import get_or_create_user, save_interaction
...
async def twilio_webhook(
    From: str = Form(...),# Número de teléfono que envió el mensaje
    Body: str = Form(...), # Texto del mensaje recibido
): 
    incoming = Body.strip()
    #Convierte el mensaje del usuario (Body) a un vector numérico usando embeddings de OpenAI.

    #Guarda ese vector en la base Chroma, junto con el texto original.
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "Si no hay suficiente contexto, pregunta para obtener más detalles.\n\n"
                    f"Contexto:\n{ctx}"
                ),
            },
